...
function addComma(number) {
  var number_parts = number + "";
  var regexp = /\B(?=(\d{3})+(?!\d))/g;
  if (number_parts.length > 1) {
    return number_parts.replace(regexp, ",");
  } else {
    return number;
  }
}
      ...
      return (
        <>
          <TableCell align="right" style={{ color: "blue" }}>
            ▼ {(row.befnum = `${addComma(row.befnum)}`)}
          </TableCell>
        </>
      );
    } else if (row.befnum > 0) {
      return (
        <>
          ...
          <TableBody>
            {(rowsPerPage > 0
              ...
              <TableRow key={row.name}>
                ...
                <TableCell align="right">
                  {(row.now = `${addComma(row.now)}`)}
                </TableCell>
                {table(row)}
                <TableCell align="right">
                  {(row.close = `${addComma(row.close)}`)}
                </TableCell>
                <TableCell align="right">
                  {(row.high = `${addComma(row.high)}`)}
                </TableCell>
                <TableCell align="right">
                  {(row.open = `${addComma(row.open)}`)}
                </TableCell>
                <TableCell align="right">
                  {(row.low = `${addComma(row.low)}`)}
                </TableCell>
              </TableRow>
            ))}
